...
def solve_burst(burst,start_timestamp, precal_func_config, hackrf_num, worker_idx, processcut1000_idx, fs = 15.36e6):
    ...
    # for i in range(10):
    #     k=512-5+i
    #     burst = freq_offset(bursttemp,precal_func_config,padding,k)
    #     scores_cp_sto = np.zeros(num_tests)
    #     for idx in range(num_tests):
    #         offset = idx
    #         scores1 = np.zeros(num_ofdm_symbols)
    ...
    #     process_list_cut=[]
    #     for mul in range(50):
    #         process_list_cut.append(threading.Thread(target=cfo_freq_offset, args=(burst,start_timestamp, precal_func_config, offset_radians,fs,mul)))
    #     for mul in range(50):
    #         process_list_cut[mul].start()
    #     for mul in range(50): 
    #         process_list_cut[mul].join()
        ...
        #     print(mul)
        #     try:
        #         command = f"cmd /c \"wsl /mnt/c/demode/cpp/remove_turbo /mnt/c/tmp/bits{mul}\""
        #         returned_text = subprocess.check_output(command, shell=True, text=True)
        #         # print("returned_text=",returned_text)
        #         # returned_text = returned_text.decode("utf-8")
        #         returned_text = returned_text.strip()
        #         returned_text = returned_text.lstrip("0")
        #         # print("returned_text=",returned_text[0:6])
        #         if returned_text[0:7] != '[ERROR]':
        #             timestamp = start_timestamp.timestamp()
        #             upload_data = framepg(returned_text,timestamp)
        #             print("upload_data=",upload_data)
        ...
        #     # offset_radians1 = offset_radians*(-3+0.025*mul)
        #     # offset_hz = offset_radians1 * fs / (2 * np.pi)
        #     offset_hz = offset_radians * fs / (2 * np.pi) + mul * 20 -1000
        #     offset_radians1 = offset_hz / fs * (2 * np.pi)
        #     # print(offset_radians1,"           ",mul)
        #     # print(offset_hz,"           ",mul)
        #     burst = np.multiply(testburst,np.exp(1j*(-offset_radians1)*np.arange(1,len(testburst)+1)))
        #     #####OFDM#####
        #     freq_domain = np.zeros((len(precal_func_config["cyclic_prefix_length_schedule"]),FFT_SIZE),dtype=complex)
        #     time_domain = np.zeros((len(precal_func_config["cyclic_prefix_length_schedule"]),FFT_SIZE),dtype=complex)
        ...
        #     channel = channel1
        #     bits = np.zeros((9,1200))
        #     DC = np.zeros((9,600),dtype=complex)
        #     for idx in range(0,len(bits)):
        #         data_carriers = freq_domain[idx,precal_func_config["data_carrier_indices"]]
        #         if enable_equalizer == 1:
        #             data_carriers = np.multiply(data_carriers,channel)
        #         DC[idx,:]=data_carriers
        ...
        #     print("find drone special signal")
        #     upload_data=[]
        #     try:
        #         returned_text = subprocess.check_output(command, shell=True, text=True)
        #         print("returned_text=",returned_text)
...
def find_burst_sol200(samples, samplesFcut,start_time,precal_func_config,numprocesscut,dataqueue,signalplace,bandplace):
    fs=15.36e6
    FFT_SIZE = int(fs/15e3)
    ...
    zcfilter_conj_fft = fft.fftshift(transform_filter["Filter_T"])# MAYBE THIS
    # zcfilter_conj_fft = transform_filter["Filter_T"]

    sliced_sample_fft = samplesFcut
    ...
    # if bandplace==1 and signalplace==1:
    #     fast_scores=1
    # print(fast_scores)
    # passing_scores=-1
    padding = 0
    if np.absolute(fast_scores)**2 > correlationscore:      #判斷1/1000的samples中correlation有無高過設定的score，倘若有則繼續解碼，反之跳出直接進入下一loop
        ...
        #print(np.absolute(fast_scores)**2)
        actual_start_index = start_index - padding - precal_func_config["zc_seq_offset"]
        # burst = true_siganl_tmp[int(actual_start_index):int(actual_start_index)+9980]
        burst = samples[actual_start_index:actual_start_index+9980]
        solve_burst(burst, start_time ,precal_func_config)
        # print(burst)
        ...
        # plt.show() 
        # dataqueue.put([bandplace,signalplace,actual_start_index])
